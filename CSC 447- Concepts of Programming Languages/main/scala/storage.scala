...
object storage {
  ...
  val getAndSet : Int => (() => Int, Int => Unit) = {
	...
	var value = x
	def get: () => Int = () => value
	def set: Int => Unit = (y: Int) => value = y
	(get,set)
	}
  }
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
    ...
    () => {
		var count =0 
		//function here getSetWithCount 
		 val getSetWithCount: Int => (() => Int, Int => Unit) = {
		 (x: Int) => {
			var value = x
			var get : () => Int = () => value
			var set : Int => Unit = (m: Int) => {count = count + 1; value = m;}
			(get,set)
		 }
		}		 
		var getCallsMade: () => Int = () => count
		var getSetSpy: Int => (() => Int, Int => Unit) = getSetWithCount
		(getCallsMade,getSetSpy)
	}
  }
